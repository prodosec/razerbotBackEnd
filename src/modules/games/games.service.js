...
async function fetchGamesList(userId, regionId) {
  ...
  const response = await axiosInstance.get(`https://gold.razer.com/api/v2/content/gold/catalogs/${regionId}`, { headers });
...
async function fetchGameDetail(userId, regionId, permalink) {
  const [headers, axiosInstance] = await Promise.all([getStoredRazerHeaders(userId), getAxiosForUser(userId)]);
  const response = await axiosInstance.get(`https://gold.razer.com/api/v2/content/gold/catalogs/${regionId}/${permalink}`, { headers });
  return response.data;
}
